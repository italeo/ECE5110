# test_newton_interpol.py

import unittest
import numpy as np
import matplotlib.pyplot as plt
from functions.newton_interpol import newton_interpolation


class TestNewtonSurfTide(unittest.TestCase):

    def test_mid_tide_prediction(self):

        # Time in hours since Feb 24, 2026 00:00
        t = np.array([
            1.92,    # Feb 24 1:55 AM (High)
            10.28,   # Feb 24 10:17 AM (Low)
        ...
        # Build Newton interpolating polynomial
        coeffs = newton_interpolation(t, h)

        # Define mid tide level
        ...
        print("\nNewton Method – Optimal Surf Windows (±30 minutes):\n")
        ...
        plt.figure()
        plt.plot(tt, P, label="Newton Interpolated Tide Curve")
        ...
        plt.title("El Porto Mid-Tide Prediction (Newton Method)")
        plt.show()

        # Verify interpolation is exact
